export function initSplashCursor(canvas) {
  if (!canvas) return;
  ...
  function drawDisplay(target) {
    let width = !target ? gl.drawingBufferWidth : target.width;
    let height = !target ? gl.drawingBufferHeight : target.height;
  ...
  function hashCode(s) { if (!s.length) return 0; let hash = 0; for (let i = 0; i < s.length; i++) { hash = (hash << 5) - hash + s.charCodeAt(i); hash |= 0; } return hash; }
